...
router.post('/:graphId', (req, res, next) => {
    if (req.user) {
        const { graphId } = req.params;
        const { xAxis, yAxis, xAxisLabel, yAxisLabel, title, graphType, shareable } = req.body;
            ...
            .then(foundGraph => {
                ...
                if (foundGraph) {
                    recentGraph = foundGraph.update({
                        userId, xAxis, xAxisLabel, yAxisLabel, title, graphType, shareable
                    })
                }
                else {
                    recentGraph = Graph.create({
                        userId, graphId, xAxis, xAxisLabel, yAxisLabel, title, graphType, shareable
                    })
                }
                return recentGraph;
            })
...
router.get('/:graphId/aws', (req, res, next) => {
    ...
    let findDatasetPromise = new AWS.S3({ apiVersion: '2006-03-01' }).getObject(datasetParams).promise();
    findDatasetPromise
        .then((result) => {
...
router.post('/:graphId/aws', (req, res, next) => {
    const { graphId } = req.params;
    const { dataset } = req.body;
    const stringifiedDataset = JSON.stringify(dataset);
    let datasetParams = { Bucket: bucketName, Key: graphId, Body: stringifiedDataset };
    //this creates or updates the desired object
    let uploadDatasetPromise = new AWS.S3({ apiVersion: '2006-03-01' }).putObject(datasetParams).promise();
